...
import freeToPlayGamesData from "../data/freeToPlayGamesData";

export default function FreeToPlayPages() {
	const [visibleRows, setVisibleRows] = useState(4); // default 4 baris (20 data)
		const [showFilter, setShowFilter] = useState(false);
		const filterRef = useRef(null);
	const maxRows = Math.ceil(freeToPlayGamesData.length / 5);
				...
				<div className="min-h-screen font-sans overflow-x-hidden overflow-y-auto" style={{background: 'linear-gradient(135deg, #3A7A7A 0%, #102425 100%)'}}>
						<div className="max-w-7xl mx-auto px-6 md:px-12 pb-2">
								...
								<div className="flex justify-between items-center pt-8">
										<div>
												<div className="flex items-center gap-2 text-xs text-white/80 mb-2">
														...
														<span className="text-[#96EDD6]">Free to Play</span>
												</div>
												<h1 className="text-4xl md:text-5xl font-bold text-white mb-2">Free to Play</h1>
												<p className="text-white/80 text-base mb-2 mt-20">Explore. Play. Repeat.</p>
										</div>
						       ...
						       <div className="flex flex-col items-center mb-8">
								...
								{/* Game Grid */}
					       {/* Grid game dengan gambar */}
					       <div className="grid grid-cols-5 gap-8 mb-8">
							   {freeToPlayGamesData.slice(0, visibleRows * 5).map((game, idx) => (
							       <div
								       key={idx}
						       ...
						       <div className="flex justify-center mb-8">
							       <button
								       ...
								       onClick={() => {
									       if (visibleRows === 4) {
										       setVisibleRows(maxRows); // tampilkan semua data
									       } else {
										       setVisibleRows(4); // kembali ke 4 baris
									       }
								       }}
							       >
